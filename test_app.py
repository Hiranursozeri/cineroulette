...
def test_ai_onerileri_yil_araligi_disindakileri_eliyor():
    """
    KRİTİK (gerçek kullanıcı şikayeti): '1990-2010 seçtim ama güncel
    filmler geldi' — sidebar'daki yıl filtresi AI önerilerine hiç
    uygulanmıyordu. Artık uygulanmalı. Fonksiyonu doğrudan test ediyoruz.
    """
    from unittest.mock import patch as mock_patch, MagicMock
    from app import fetch_ai_recommendations
    from ml.recommendation_engine import RecommendationEngine

    old_and_new = [
        {"id": 10, "title": "Eski Film", "vote_average": 7.5, "overview": "Bir macera hikayesi.",
         "release_date": "1995-01-01", "content_type": "movie", "genre_ids": [12]},
        {"id": 11, "title": "Guncel Film", "vote_average": 7.5, "overview": "Bir macera hikayesi.",
         "release_date": "2026-01-01", "content_type": "movie", "genre_ids": [12]},
    ]
    favorites = [{"id": 1, "title": "Favori", "overview": "Macera dolu bir hikaye.", "genre_ids": [12], "content_type": "movie"}]

    tmdb = MagicMock()
    tmdb.discover_movies.return_value = old_and_new
    tmdb.get_popular_movies.return_value = old_and_new

    engine = RecommendationEngine()
    result = fetch_ai_recommendations(
        tmdb=tmdb,
        ml_engine=engine,
        favorites=favorites,
        content_type="movie",
    ...
    result_titles = [r.get("title") for r in result]
    assert "Guncel Film" not in result_titles, "2026 filmi, 1990-2010 aralığı seçiliyken çıkmamalıydı"
    assert "Eski Film" in result_titles


def test_puan_yil_sure_url_yaziliyor():
    """Puan/yıl/süre kaydırıcıları değiştirildiğinde URL query param'larına da yazılmalı."""
    at, patches = _mocked_app()
    try:
        assert "rating" in at.query_params
        assert "year" in at.query_params
        assert "runtime" in at.query_params
    finally:
        _stop_patches(patches)


def test_puan_yil_sure_urlden_geri_yukleniyor():
    """Sayfa, URL'de puan/yıl/süre parametreleri varken açılırsa, bunları geri yüklemeli (F5 sonrası kaybolmamalı)."""
    discover_return = make_fake_items()
    with patch("utils.tmdb_client.TMDBClient._validate_api_key", return_value=None), \
         patch("utils.tmdb_client.TMDBClient.discover_movies", return_value=discover_return), \
         patch("utils.tmdb_client.TMDBClient.discover_tv_shows", return_value=discover_return):
        at = AppTest.from_file("app.py")
        at.query_params["mode"] = "mood"
        at.query_params["moods"] = "aglamalik"
        at.query_params["rating"] = "7.5-9.5"
        at.query_params["year"] = "2000-2015"
        at.query_params["runtime"] = "30-120"
        at.run(timeout=30)

        assert len(at.exception) == 0
        rating_slider = next((s for s in at.slider if s.key == "rating_range_slider"), None)
        year_slider = next((s for s in at.slider if s.key == "year_range_slider"), None)
        runtime_slider = next((s for s in at.slider if s.key == "runtime_range_slider"), None)

        assert rating_slider is not None and rating_slider.value == (7.5, 9.5)
        assert year_slider is not None and year_slider.value == (2000, 2015)
        assert runtime_slider is not None and runtime_slider.value == (30, 120)
